...
void CairoPsRenderer::Setup(wxBitmap *output, DRect const &viewport)
{
			m_offX = viewport.m_x;
			m_offY = viewport.m_y;
			m_scaleX = m_width / viewport.m_w;
...
void CairoPsRenderer::Begin(Renderer::TYPE type, int layer)
{
	m_type = type;
	cairo_new_path(m_context);
}
...
void CairoPsRenderer::AddPoint(double x, double y, double xshift, double yshift)
{
	cairo_line_to(m_context, (x - m_offX) * m_scaleX + xshift, m_height - (y - m_offY) * m_scaleY + yshift);
}

void CairoPsRenderer::End()
{
	switch(m_type)
	{
		case R_POLYGON:
...
void CairoPsRenderer::DrawCenteredText(char const *text, double x, double y, double angle, int r, int g, int b, int a, int layer){ }
